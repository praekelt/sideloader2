...
test_config = {
    ...
    'queues': [{
        'id': 0, 'max_jobs': 1,
        'name': 'sideloader',
        'plugin': 'hive.plugins.sideloader'
    }],
    'redis_port': 6379
}
...
class Tests(unittest.TestCase):
    def setUp(self):
        self.service = service.HiveService(yaml.dump(test_config))
    ...
    def test_heartbeat(self):
        ...
        hb = self.client.kv.get("hive.server.%s.heartbeat" % self.service.hostname)
    ...
    def test_status(self):
        ...
        hb = self.client.kv.get("hive.server.%s.status" % self.service.hostname)
